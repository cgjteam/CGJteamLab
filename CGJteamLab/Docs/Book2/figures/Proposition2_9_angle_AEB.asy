...
void anglearc(pair O, pair P, pair Q, real r, pen p=lightpen) {
  ...
  while(a2<a1) a2+=360;
  if(a2-a1>180) { real tmp=a1; a1=a2; a2=tmp+360; }
  draw(arc(O,r,a1,a2),p);
}

pair A=(-4,0), C=(0,0), B=(4,0), E=(0,4);
real t=(1.45-E.x)/(B.x-E.x);
pair F=E+t*(B-E);
...
dotlabel("A",A,SW); dotlabel("C",C,S); dotlabel("B",B,SE); dotlabel("E",E,N); dotlabel("F",F,NE);
label("I.5 + I.32",(0,4.65),N);
label("$\angle AEC\cong\angle BEC$",(0,3.0),S);
label("$\angle AEB$ right",(0,2.45),S);
label("$E-F-B$",(3.05,1.45),E);

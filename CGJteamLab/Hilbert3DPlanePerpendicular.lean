...
/-!
# Perpendicular planes in Hilbert 3-space

Synthetic plane-plane perpendicularity in the language of Euclid XI.Def.4.

This module is deliberately neutral with respect to the two Book XI proof
routes. It belongs to the common Hilbert 3D interface and may be used both by

* the direct Hilbert reconstruction of Book XI, and
* the Wyler reconstruction used as a transfer laboratory toward E^4.

The definition follows Euclid XI.Def.4 rather than replacing it by the modern
equivalent statement that one plane contains a normal to the other. That
distinction is essential for Proposition XI.18: otherwise XI.18 would become
true by definition and its geometric content would disappear.

The relation is directed. Symmetry, if needed later, is to be proved as a
theorem.
-/

/--
`rho` is perpendicular to `pi` along their common section line `s` when:
...
/--
Two distinct planes are perpendicular when they have a common section line
satisfying Euclid XI.Def.4.

The order of arguments is significant: `rho` is the plane in which the lines
perpendicular to the common section are tested, while `pi` is the plane to
which those lines must be perpendicular.
-/

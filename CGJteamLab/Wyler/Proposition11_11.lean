...
From a point A outside a plane pi, construct a straight line through A
perpendicular to pi.

The Wyler reading is explicit inside the proof:

  A outside pi
      ->
  Join(carrier(pi), {A}) = E^3

and the remaining body carries out Euclid's actual synthetic
perpendicular construction through planar slices, I.12, I.31, XI.4 and
XI.8.  The rank-three certificate is therefore a structural input to the
construction, not a replacement for XI.11.
-/
theorem euclid_proposition_11_11
  ...
  /-
  Flat rank certificate for the external point A.

      A outside pi
          =>
      Join(carrier(pi), {A}) = E^3.

  XI.11 then performs the actual perpendicular construction below.
  -/
  have hRank3 :
      HilbertJoin3D Geo

...
/-!
# Hilbert-Wyler metric core

This module extracts the reusable metric engine first developed in the
proof of Euclid XI.4.

It is deliberately below the numbered Book XI proposition modules:
there is no import of `Proposition11_*`.

The main exported endpoint for the Wyler layer is

  hilbert_XI4_line_perpendicular_plane_core

which proves perpendicularity to a plane from perpendicularity to two
distinct generating directions in that plane.

The current helper names retain the XI4 prefix because this is the first
dependency-inversion pass.  Semantic renaming can be done after the new
...
Wyler-layer version promoted from the XI.5 infrastructure.
-/
theorem hilbertLinePerpendicularPlaneAt_not_in_plane_wyler
    [H : HilbertIncidence Geo]
    [S : HilbertSpacePrimitive Geo]
    (l : Geo.Line)
    (pi : S.Plane)
...
/--
Symmetry of ambient line-line perpendicularity.

Wyler-layer version promoted from the XI.6 infrastructure.
-/
theorem hilbertLinesPerpendicularAt_symm_wyler
    [H : HilbertIncidence Geo]
    [HilbertPlaneIncidence Geo]
    [S : HilbertSpacePrimitive Geo]
    [HSI : HilbertSpaceIncidence Geo]

...
two pairs of opposite strict parallel sides have a common
midpoint of their diagonals.
-/
axiom common_midpoint_of_two_parallel_pairs
    [TarskiNeutral Geo]
    (A B C D : Geo.Point)
...
theorem parallelogram_of_two_parallel_pairs
    [TarskiNeutral Geo]
    (A B C D : Geo.Point)
    (hABCD : TarskiParallelStrict Geo A B C D)
    (hBCAD : TarskiParallelStrict Geo B C A D) :
  ...
  rcases hABCD with ⟨hAB, hCD, hNoIntABCD⟩
  ...
  rcases
    common_midpoint_of_two_parallel_pairs
      Geo A B C D
      ⟨hAB, hCD, hNoIntABCD⟩
      hBCAD
  with ⟨M, hMAC, hMBD⟩

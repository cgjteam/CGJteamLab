...
theorem i36_intermediate_EBCH
    [HilbertIncidence Geo]
    [HilbertEuclideanPlane Geo]
    (B C E H M : Geo.Point)
    (hEH_BC : Geo.Parallel E H B C)
    (hEH_BC_cong : Geo.Congruent E H B C)
    (hEMC : Geo.Between E M C)
    (hHMB : Geo.Between H M B) :
    IsParallelogram Geo E B C H := by

  --------------------------------------------------------------------
  -- I.33 applied to the equal parallel segments EH and BC.
  --
  -- The crossing data are
  --
  --   E-M-C
  --   H-M-B.
  --
  -- Hence I.33 gives EB || HC.
  --------------------------------------------------------------------
  ...
  --------------------------------------------------------------------
  -- The other pair is inherited directly from EH || BC:
  --
  --   BC || HE.
  --------------------------------------------------------------------
...
theorem i36_intermediate_HBCE
    [HilbertIncidence Geo]
    [HilbertEuclideanPlane Geo]
    (B C E H M : Geo.Point)
    (hEH_BC : Geo.Parallel E H B C)
    (hEH_BC_cong : Geo.Congruent E H B C)
    (hEMB : Geo.Between E M B)
    (hHMC : Geo.Between H M C) :
    IsParallelogram Geo H B C E := by

  --------------------------------------------------------------------
  -- Reverse EH to HE so that I.33 is applied to HE and BC.
  --------------------------------------------------------------------

  have hHE_BC :
      Geo.Parallel H E B C :=
    ParallelSwapFirstLine
      Geo E H B C hEH_BC

  have hHE_BC_cong :
      Geo.Congruent H E B C :=
    CongruentReverseFirst
      Geo E H B C hEH_BC_cong
  ...
  have hHB_CE :
      Geo.Parallel H B C E :=
    ParallelSwapSecondLine
      Geo H B E C hHB_EC

  --------------------------------------------------------------------
  -- The second pair is simply BC || EH.
  --------------------------------------------------------------------
...
theorem i36_from_intermediate_HBCE
    [HilbertIncidence Geo]
    [HilbertEuclideanPlane Geo]
    (A B C D E F G H : Geo.Point)
    (hABCD : IsParallelogram Geo A B C D)
    (hEFGH : IsParallelogram Geo E F G H)
    (hHBCE : IsParallelogram Geo H B C E)
    (hADE : Geo.Between A D E)
    (hAHE : Collinear Geo A H E)
  ...
  have hFirst :
      HilbertScissorsEquicomplementable Geo
        (hilbertParallelogramTerm Geo A B C D)
        (hilbertParallelogramTerm Geo H B C E) :=
    euclid_proposition_35
      Geo
      A B C D H E
      hABCD
  ...
  have hBHEC :
      IsParallelogram Geo B H E C :=
    ParallelogramReverse
      Geo H B C E hHBCE

  have hGHEF :
      IsParallelogram Geo G H E F :=
    ParallelogramRotateTwo
      Geo E F G H hEFGH
  ...
  have hSecond :
      HilbertScissorsEquicomplementable Geo
        (hilbertParallelogramTerm Geo B H E C)
        (hilbertParallelogramTerm Geo G H E F) :=
    euclid_proposition_35
      Geo
      B H E C G F
      hBHEC
...
theorem euclid_proposition_36_crossing
    [HilbertIncidence Geo]
    [HilbertEuclideanPlane Geo]
    (A B C D E F G H M : Geo.Point)
    (hABCD : IsParallelogram Geo A B C D)
    (hEFGH : IsParallelogram Geo E F G H)
    (hBCFG : Geo.Congruent B C F G)
    (hADE : Geo.Between A D E)
  ...
  --------------------------------------------------------------------
  -- Normalize the two ordered carrier lines:
  --
  --   A-D-E-H  gives  A-D-H and A-E-H
  --   B-C-F-G  gives  B-C-G and B-F-G.
  --------------------------------------------------------------------
  ...
  --------------------------------------------------------------------
  -- I.33 in the explicit crossing configuration:
  --
  --   E-M-C
  --   H-M-B
  --
  -- gives the intermediate parallelogram EBCH.
  --------------------------------------------------------------------

  have hEBCH :
      IsParallelogram Geo E B C H :=
    i36_intermediate_EBCH
      Geo
      B C E H M
      hEH_BC.1
      hEH_BC.2
  ...
  --------------------------------------------------------------------
  -- I.35 twice + transitivity of equicomplementability.
  --------------------------------------------------------------------

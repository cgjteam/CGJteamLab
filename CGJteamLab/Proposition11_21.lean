...
/-!
# Euclid XI.21 -- Hilbert route

The three plane angles containing a proper trihedral solid angle are
together less than four right angles.

The proof is entirely synthetic and does not introduce numerical angle
measure or a general addition operation on angles.

Instead:

1. extend AC through A to X and AD through A to Y;
2. replace the ray AC by its opposite ray AX;
3. reorder the resulting trihedral configuration as A;D,X,B;
4. apply XI.20:
       angle DAX + angle XAB > angle DAB;
5. use vertical angles:
       angle DAX ~= angle CAY;
6. use symmetry of the unoriented angle:
       angle XAB ~= angle BAX;
7. transport the XI.20 comparison to
       angle CAY + angle BAX > angle DAB.

By the common complement representation in
`HilbertThreeAnglesFourRight.lean`, this is exactly the XI.21 bound.
-/


/--
Construct the two opposite rays used in the complement representation:

    C - A - X,
    D - A - Y.
-/
theorem hilbert_XI21_supplement_rays_exist
...
/--
Replacing the ray AC by its opposite ray AX preserves the proper
trihedral configuration.

The only spatial step is noncoplanarity: if A,B,X,D were coplanar,
then the line AX would lie in that plane, hence C would lie there as
well because C,A,X are collinear.
-/
theorem hilbert_XI21_trihedral_opposite_middle
    [HilbertIncidence Geo]
    [HilbertOrder Geo]
    [S : HilbertSpacePrimitive Geo]
...
-/
theorem hilbert_XI21_trihedral_reorder
    [HilbertIncidence Geo]
    [S : HilbertSpacePrimitive Geo]
    (A B X D : Geo.Point)
    (h :
...
congruent.
-/
theorem hilbert_XI21_vertical_DAX_CAY
    [HilbertIncidence Geo]
    [HilbertCongruence Geo]
    (A C D X Y : Geo.Point)
    (hDAX :
...
-/
theorem hilbert_XI21_angle_XAB_BAX
    [HilbertIncidence Geo]
    [HilbertCongruence Geo]
    (A B X : Geo.Point)
    (hXAB :
...
/--
The direct complement comparison for XI.21:

    angle CAY + angle BAX > angle DAB.
-/
theorem hilbert_XI21_direct_comparison
    [HilbertIncidence Geo]
    [S : HilbertSpacePrimitive Geo]
    [HilbertSpaceIncidence Geo]
    [HilbertCongruence Geo]
  ...
  have hTriBX :
      HilbertTrihedralConfiguration
        Geo A B X D :=
    hilbert_XI21_trihedral_opposite_middle
      Geo
      A B C D X
      hTri
      hCAX
  ...
  have hTriDXB :
      HilbertTrihedralConfiguration
        Geo A D X B :=
    hilbert_XI21_trihedral_reorder
      Geo
      A B X D
      hTriBX
...
together less than four right angles.
-/
theorem euclid_proposition_11_21
    [HilbertIncidence Geo]
    [S : HilbertSpacePrimitive Geo]
    [HilbertSpaceIncidence Geo]
    [HilbertCongruence Geo]
  ...
  rcases
      hilbert_XI21_supplement_rays_exist
        Geo
        A B C D
        hTri
    with
  ...
  have hCompare :
      HilbertTwoAnglesGreaterThanAngle
        ...
        B A X
        D A B :=
    hilbert_XI21_direct_comparison
      Geo
      A B C D X Y
      hTri
      hCAX
